...
## 十一、小红书全自动流水线 v2（2026-08-21 启用，Notion 新模板）

> **来源**：Notion 新包 `XHS_Auto_Agent_Blueprint` + `GitHub Pipeline Case`（已归档到 `template_cover_v2.html` / `template_repo_v2.html` / `agent_instructions_v2.md` / `工作案例_全自动流水线.md`）。**核心改进是把「囤稿能力」锁进模板，而不是靠人记住。**

### v2 模板变更（与 v1 的区别，必须知道）

| 项 | v1（2026-08-15 fable5） | v2（当前，为囤稿优化） | 为什么改 |
|---|---|---|---|
| 封面底色 | 白底 `#FFFFFF` + 蓝字 | **克莱因蓝 `#002FA7` 底 + 白字** | 小红书信息流里蓝色块识别度更高，品牌一致性更强 |
| 封面日期 | 右上角 `08 · XX` | **已删除** | 方便随时囤稿、打乱发布顺序不违和（Fable 5 建议的核心） |
| 项目卡 | 白底，无顶部色条 | **白底 + 顶部 16px 实色蓝条 `#002FA7`** | 视觉锚点，防止盗图时被裁掉品牌色 |
| 水印 | 页脚 `YUE GENG 悦 · GitHub 日榜` | 同 v1，**不可移除** | 防盗图，统一归属 |

**结论：以后所有小红书出图统一用 v2 模板（`template_*_v2.html`）。v1 模板仅作历史参照保留，不再用于新生产。**

### 双层流水线分工（SOP，已落地）

```
数据 Agent（本仓库，本地）          渲染 Agent（Notion，云端）
scraper.py → inject_zh →           agent_instructions_v2.md + template_*_v2
gen_material.py                     → headless 截图 1242×1660
   ↓                                   ↓
GitHub经典项目资料库-日期.md     →  封面1 + 项目卡3 + xhs_post.txt
   ↓                                   ↓
      └───────── ZIP 打包（一键下载） ──────────┘
```

### 标准工作流指令（可直接复制给 Notion 的执行 Agent）

> 见 `agent_instructions_v2.md` 原文。核心 5 步：1) 替换 `{{...}}` 占位符 2) `chromium --headless --window-size=1242,1660 --screenshot` 3) 生成 `xhs_post.txt`（≤150 字） 4) `zip -j` 打包 5 张图 + 1 文案 5) 交付下载链接。

### 可复用经验（为什么这次能自动）

1. **渲染高确定性**：弃用文生图，HTML+CSS 当模具，Headless Chromium 精准截图，尺寸/字体/颜色零漂移。
2. **防衰变设计**：模板去日期、页脚锁水印，内容与时间解耦，才能囤 30 期。
3. **去 AI 味是调性**：四字段法 + 数字进数据行 + 感叹号 ≤1 + 判断句给观察（详见本文件第十节编辑腔铁律）。
4. **人只做终审**：Agent 产出 → 人每天 ≤5 分钟否决制审核（数据不一致/禁词/标题看不懂才改）。

### 30 期富化素材（当前库存，Notion 可直接取用）

- **富化终版（去 AI 味）**：`xhs-assets/选题/30期选题清单-富化终版.md`（30 期 × 3 项目 = 90 卡，每条定位均含具体功能/数字/场景，0 车轱辘话，de-ai-flavor 扫描通过）
- **打包**：`../GitHub选题-30期-富化终版-去AI味-2026-08-20.zip`（14K）
- **质量**：90 条定位 0 空、封面标题 30 个差异化、数字覆盖率视项目而定，批量质检已通过。
